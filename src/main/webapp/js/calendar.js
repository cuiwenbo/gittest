...
strFrame+='        onclick="parent.meizzNextM()" title="往后翻 1 月" ><b >&gt;</b></td>';
strFrame+='      </tr>';
...
{
 if (arguments.length > 2){alert("对不起！传入本控件参数太多！");return;}
 if (arguments.length == 0){alert("对不起！您没有传入本控件任何参数");return;}
...
function tmpSelectSecondInnerHTML(strSecond) //秒的下拉框
{
 ...
 if (strSecond.match(/\D/)!=null){alert("秒钟输入参数不是数字！");return;}
