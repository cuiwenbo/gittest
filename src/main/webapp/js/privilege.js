...
function deleFunctionComp(functionId, functionName){
   ...
   if(confirm("是否确定删除 "+functionName) == true){
      ...
      var myAjax= new Ajax.Request("priviAction!deleteFuncitonComp.action",{method:'post',parameters:para,onComplete:deleFunctionPara,onError:error}); 
   }
}
...
function deleFunctionPara(response){
	...
	if("ok" == ret.deleResult) {
		window.location.href="priviAction!index.action";
	} else {
...
function deleFunctionGroup(groupId,groupName){
   ...
   if(confirm("是否确定删除 "+groupName)==true){
      ...
      var myAjax= new Ajax.Request("priviAction!deleteFuncitonGroup.action",{method:'post',parameters:para,onComplete:deleFunctionGroupPara,onError:error});
   }
}
...
function deleFunctionGroupPara(response){
	 var ret = eval('(' + response.responseText + ')');
	if("ok" == ret.deleResult) {
		window.location.href="priviAction!functionGroupsMana.action";
	} else {
	  var deleFuncState = document.getElementById("deleFuncState");
	  showInfo("deleFuncState",funName+" 已授权，不能删除! ",true);
	}
