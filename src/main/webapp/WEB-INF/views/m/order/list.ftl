...
<link href="${rc.contextPath}/m/css/pager.css"rel="stylesheet" type="text/css" />
...
<script>
	...
	function getRecept(id){
		if(confirm("确认收货？？")){
			$.ajax({
				...
				url:"${rc.contextPath}/m/order/Receipt?id=${order.id}",
				dataType: "json",
	...
	function cancel(id){
		...
		$.ajax({
				...
				url: "${rc.contextPath}/m/order/cancelOrder?id=${order.id}",
				dataType: "json",
				success: function(ret){
					if('0000' == ret.result) {
						alert(ret.message);
...
<div class="container">
 	<form name="searchfrm" action="${rc.contextPath}/m/order/start?sid=${sid}" method="get" >
	 	...
	 	<div class="row">
            ...
            <ul id="myTab" class="nav nav-tabs ode-tab" style="margin-top:10px;border-top:1px solid #d5d5d5;">
         		  <li <#if state==-2>class="active"</#if> style="width:100%">
         		  	<a href="${rc.contextPath}/m/order/getOrder?sid=${sid}" style="text-decoration:none;text-align:left;padding-left:5%;padding-top:0;padding-bottom:0;border-bottom:1px solid #d5d5d5;">全部订单</a></li>
            </ul>
            <div id="myTabContent" class="tab-content">
               <div class=" tab-pane fade in active" id="home">
   					...
   					<#list page.elements as order>
		                   ...
		                   <a href="${rc.contextPath}/m/order/getOrder?id=${order.id}" style="position:relative">
		               	 ...
		               	 <div class="col-xs-12 ode-the">
		                        ...
		                        <div class="col-xs-9 ode-isa">
		                        		 <span style="position:absolute;top:35px;right:10px;border-radius:10px;padding:0;width:20px;height:20px;display:block;background:#b8b8b8;">
		                        		 	<img style="padding:0 0 4px 3px;" src="${rc.contextPath}/m/img/arrow.png" />
		                        		 </span>
