...
<link type="text/css" href="${rc.contextPath}/css/shoplist.css" rel="stylesheet" />
<script src="${rc.contextPath}/js/jquery-1.8.2.js"></script>

<script src="${rc.contextPath}/js/add_pic.js"></script>
...
<div class="main">
	<div class="main_title">
		<h1>选择公司/店铺</h1><h2>${user.phone} - <a href="user/logout">退出</a></h2>
	</div>
	<div class="main_contain">
		<div class="mer_pic">
			<#if user.pict?? && user.pict?length gt 0>
				<img src="${imghost}${user.pict}" width="80" height="80"/>
			<#else>
				<img src="${rc.contextPath}/images/head-pic.png" width="80" height="80"/>
			</#if>
		</div>phone
		...
		<div class="mer_info">
			...
			<a href="${rc.contextPath}/user/getUserInfo" target="workwindow">设置</a><a href="${rc.contextPath}/shop/start" target="workwindow">创建新的店铺</a>
		</div>
	</div>
	<div class="shop_list">
		<#if slistsize?? && slistsize &gt; 0>
			<#list slist as s>
					...
					<div class="mer_shop">
						<div class="shop_name">
							...
							<a href="shop/getShop?sid=${s.id}" target="workwindow">修改</a>
						...
						<div class="shop_pic">
							...
							<#else>
								<img src="${rc.contextPath}/images/shop-pic0.png" width="80" height="80"/>
							</#if>
						</div>
						<div class="shop_info">
							<p>
								<#if (s.authed==1)>
									...
									<a  target="workwindow" href="${rc.contextPath}/goods/goodsList?sid=${s.id}">商品管理</a>
								<#else>												
									<#if (s.authed == 0 || s.authed == 3)>
									<a  target="workwindow" href="${rc.contextPath}/shop/get?op=shopCertification&sid=${s.id}">提交认证资料</a>
									<#else>
			...
			<a  target="workwindow" href="shop/start">
				<div class="add_shop">
					添加店铺
				</div>
			</a>
...
function setChange(){
		...
		var url = "${rc.contextPath}/user/setLoginPhone";
			$.ajax({
    ...
    function sendPhoneCode() {
		...
		var url = "${rc.contextPath}/mobilecode/send";
		var par = "phone=" + phone;
			$.ajax({
