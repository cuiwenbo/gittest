...
<style>
	...
	.delete_pic{position:absolute;border:none;background:url(${rc.contextPath}/images/xx.png);width:20px;height:20px;top:-9px;left:100px;border:none;line-height:18px;color:white;font-size:13px;opacity:0.8;filter:alpha(opacity=80)}
...
<script>
	...
	function AfterOperate(param) {	
		...
		if(param.op =="edit")  return afterSave(param);
    	...
    	window.onload =  function() {
    		 $("input[type='radio'][name='deliverymethod'][value='${g.deliverymethod}']").attr("checked",true);  //送货方式
    		 $("input[type='radio'][name='paytype'][value='${g.paytype}']").attr("checked",true);  //支付方式
    		 
    		picts = "${g.picts}".split(",");    		
    		var path  = "${imghost}";
    		if(picts.length > 0) mainPictIdx = 0;
    		for(var idx = 0; idx < picts.length; idx ++) {
    			if(picts[idx].length == 0) continue;
    			var pict = path + picts[idx];
...
<div class="header">
	...
	<div class="contain" id="contain">
		...
		<div class="main_right"  id="main_right" style="background:#fbfbfb">
			...
			<form action="${rc.contextPath}/goods/editgoods?id=${g.id}" method="post" target="hidefrm" name="myForm" onsubmit="return checkData()">
					...
					<div class="step_one">
						<div class="basic_info" style="height:240px">
							...
							<div class="shop_main" style="height:240px">
								<p style="margin-top:15px;">
									...
									<input class="txt" type="text" autocomplete="off" id="name" name="name"  value="${g.name}"/>
									<span class="error"></span>
								</p>
								<p>
									<label><em>*</em>商品价格：</label>
									<input class="txt" type="text" autocomplete="off" id="price" name="price"  value="${g.price}"/>
									<span class="error"></span>
								</p>
								<p>
									<label>商品原价：</label>
									<input class="txt" type="text" autocomplete="off" id="oprice" name="oprice" value="${g.oprice}">
									<span>（不写则表明原价与现在价格相等）</span>
								</p>
								<p>
									<label><em>*</em>商品库存：</label>
									<input class="txt" type="text" autocomplete="off" id="amount" name="amount" value="${g.amount}"/>
									<span class="error"></span>
								</p>
								<p>
									<label>限购数量：</label>
									<input class="txt" type="text" autocomplete="off" id="userlimit" name="userlimit"  value="${g.userlimit}"/>
									<span>（0 代表不限购）</span>
								</p>
							</div>
						...
						<div class="basic_info">
							...
							<div class="shop_main">
								<p style="margin-top:15px;">
									<label>商品单位：</label>
									<input class="txt"  type="text" autocomplete="off" id="unit" name="unit" value="${g.unit}"/><span class="error"></span>
								</p>
								<p>
									<label>商品规格：</label>
									<input type="hidden" name="param" id="param"/>
									名称: <input class="standard" type="text" autocomplete="off" name="pn1" value="${g.getPn(0)}" />
									参数值: <input class="standard"  type="text" autocomplete="off" name="pv1" value="${g.getPv(0)}" /> <span>(参数值之间用,分割)</span>
								<p>
									<label></label>
									名称: <input class="standard"  type="text" autocomplete="off" name="pn2" value="${g.getPn(1)}" />
									参数值: <input class="standard"  type="text" autocomplete="off" name="pv2" value="${g.getPv(1)}" /> <span>(例如：名称：尺寸 )</span>
								</p>
								<p>
									<label></label>
									名称: <input class="standard" type="text" autocomplete="off" name="pn3" value="${g.getPn(2)}" />
									参数值: <input class="standard" type="text" autocomplete="off" name="pv3"  value="${g.getPv(2)}" /> <span>(例如：参数值：S，M，L，XL，XXL )</span></p>
						...
						<div class="basic_info">
							...
							<div class="shop_main">
								<p>
									...
									<input type="radio" name="paytype" id="xianxia" value="线下支付"> 线下支付
									<span> (使用线下支付，则可自行选择支付方式，如支付宝转账，面对面交易等)</span>
								</p>
								<p>
									<label>送货方式：</label>
									<input type="radio" name="deliverymethod" class="d" id="express" checked value="0" onclick="setFee(this.value);">  快递
									<input type="radio" name="deliverymethod" class="d" id="byself" value="1" onclick="setFee(this.value);"> 上门提货
								</p>
								<p>
									<label>送货金额：</label>
									<input class="txt" type="text" autocomplete="off" id="deliveryfee" name="deliveryfee"  value="${g.deliveryfee}"/> 元 <span class="error"></span>
			...
			<div id="add_second" style="display:none">
				...
				<div class="step_two">
					...
					<div class="goodscontent">
						...
						<textarea id="content" name="content"  style="width:550px;border:1px solid #ccc;height:450px;visibility:hidden;resize:none;max-width:545px">${g.content}</textarea>
					</div>
