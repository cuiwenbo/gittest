...
function update(){
	...
	$.ajax({
			...
			url:"${rc.contextPath}/shop/editBankNo",
			dataType:"json",
			success:function(ret){
				if(ret.result=='0000'){
                    alert(ret.message);
	...
	function draw(){
		$.ajax({
			...
			url:"${rc.contextPath}/order/getDraw?sid=${shop.id}",
			dataType:"json",
			success:function(ret){
				if(ret.result=='0000'){
				 	window.location="${rc.contextPath}/order/cashList?sid=${shop.id}&op=cashRecord";
				}
				alert(ret.message);
			}
		});		
		...
		<div class="contain_right" id="contain_right">
			<div id="all" style="display:block;">
				...
				<div class="shop_payinfo">
						...
						<#else>
							<img src="${rc.contextPath}/images/shop-pic0.png"/>
						</#if>
				...
				<div class="income">
					<div class="seven_in">
						<p>
							最近7天收入            
							<a href="${rc.contextPath}/order/cashList?sid=${shop.id}&op=paymentDe">查看收支明细</a>
						</p>
						<p class="in_money">
							<img src="${rc.contextPath}/images/money.png" width="28" height="28" style="vertical-align:top;">
				...
				<div class="capital">
					...
					<#if op=='lastSale'>
						<div class="record">
							<span>最近交易记录</span>
							<a href="${rc.contextPath}/order/getFinance?sid=${shop.id}&op=refundRecord">退款记录</a>
						</div>
						<div id="d1">
							<div class="heading">
					...
					<#if op=='refundRecord'>
						<div class="record">
							<span>退款记录</span>
							<a href="${rc.contextPath}/order/getFinance?sid=${shop.id}&op=lastSale">最近交易记录</a>
						</div>
						<div id="d2">
							<div class="heading">
		...
		<!-- 提现 -->
			<div id="draw" class="withdraw_out" style="display:none;">
				<div class="draw_title">
					...
					<span>
						<a href="${rc.contextPath}/order/cashList?sid=${shop.id}&op=cashRecord"">提现记录</a>
					</span>
				</div>
